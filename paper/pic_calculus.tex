...
\begin{definition}[Sources, incidence, aggregate]
A \emph{source set} $S$ ($|S|=J$) assigns to each source $j$ a contribution $c_j:V\to R$ valued in a
semiring carrier $R$. The \emph{residual reading} fixes a vector $d_j\in\HH$ per source and sets
\[
  c_j(v)=\inner{d_j}{U_v}.
\]
The \emph{aggregate score} of $v$ is the semiring sum of its incidences with a bias,
\[
  L(v)=\Bigl(\bigoplus\nolimits_{T,\,j\in S} c_j(v)\Bigr)\otimes b_v,
  \qquad\text{classically}\quad L(v)=\sum_{j\in S}\inner{d_j}{U_v}+b_v .
\]
\end{definition}

In the transformer instance, the sources are the \emph{DLA blocks}: an $L$-layer model has
$n_b=2L+1$ blocks (one attention and one MLP/expert write per layer, plus the embedding), and $d_j$ is
block $j$'s additive contribution to the residual at the decode position. The incidence matrix
$c_j(v)=\inner{d_j}{U_v}$ is precisely the per-block contribution a decomposing runtime emits, and its
column sum reconstructs the model's decode logit.
...
The tropical end is a genuine semiring; in particular the distributive law
$a\otimes(b\oplus c)=(a\otimes b)\oplus(a\otimes c)$ holds \proved{}, the law that rewrites a
...
\begin{itemize}[nosep]
  \item \emph{Aggregate:} $L(v)=\bigoplus_{T,\,j\in S} c_j(v)$.
  \item \emph{Retrieved:} $v$ has high margin and high multiplicity $\mu_t$ (many sources already vote
        for it).
  \item \emph{Composed:} $\mu_t=0$ (no single source decides $v$) yet $v$ wins the aggregate---the
        decision is carried by a coalition.
  \item \emph{Irreducible:} a composed $v$ over coalition $S$ such that $S$ decides $v$ but no proper
        nonempty subset does.
\end{itemize}
...
\subsection{Frame side: decode capacity}
...
\subsection{Generator side: routing rank and Welch interference}
...
natural conjecture, $\tau^\star\approx\min(e^{H},d)$ (effective output support sets the rank), is only
partly borne out. Measured on a Pythia scale ladder and Qwen models via the analysis seam, the effective
decode-block count \emph{tracks capacity} (block count, hidden width, depth) and \emph{anti-correlates}
with $e^{H}$ across scale: larger, more confident models use \emph{more} effective blocks, not fewer. A
genuine but modest \emph{within-model} effect survives---higher-entropy tokens use somewhat more blocks.
So $\tau^\star$ is real but capacity-bound rather than entropy-bound; its functional form is \open{}. We
record this as the cleanest current example of the discipline the tags enforce: a plausible law,
measured, and reported by what the data support rather than by what would have been tidy.
