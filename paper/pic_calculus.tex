...
\emph{refuted}, robustly, on a fourteen-model sweep to 72B (Pythia 14m--12b, Qwen 0.5B--72B; reconstruction
$1.00$ throughout). The architecture-fair effective decode-block count $r_{\mathrm{eff}}$ \emph{anti}-correlates
...
more confident models use \emph{more} effective blocks, not fewer.
...
$k^\star/L$ (the smallest layer prefix whose recompute already reproduces the decode), which declines with
scale in Pythia but not Qwen (Figure~\ref{fig:arch}B)---so it is a property of the models, not an artifact
of the attribution measure. So $\tau^\star$ is capacity-associated rather than entropy-bound, with
...
\begin{figure}[t]
  ...
  $0.10$. \textbf{(B)} \emph{Causal}: the convergence depth $k^\star/L$ (\texttt{--converge-depth}) ---
  the smallest layer prefix whose recompute reproduces the decode. Deep computation is the norm
  ($k^\star/L\geq0.8$); Pythia's fraction declines with scale while Qwen's does not. That two unrelated
