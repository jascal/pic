...
\begin{itemize}[nosep]
  \item \emph{Log-semiring $\Rightarrow$ transformer.} The sources are block writes with the final
        ...
        pre-norm residual and frozen, and $g$ the learned gain. For RMSNorm, $\tilde d_b=s\,g\odot d_b$.
        For LayerNorm, $\tilde d_b=s\,g\odot(d_b-\operatorname{mean}(d_b))+\mathbf 1_{b=\mathrm{embed}}\,\beta$:
        centring is per block (the means add up to the full-residual mean), and the norm bias $\beta$ is
        assigned by convention to the embedding block. Then $\sum_b\tilde d_b$ is the normed residual. Both
