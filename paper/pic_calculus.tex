...
\author{}
\date{Draft --- companion to \texttt{spec/PIC\_SPEC.md} v0.3}
...
\end{itemize}
A joint positive rescaling preserves decisions but acts as a temperature change. Independent per-token
rescaling $U_v\mapsto\lambda_vU_v$ does \emph{not} preserve decisions in general. The cosine Gram, coherence
...
\begin{itemize}[nosep]
  \item \emph{Log-semiring $\Rightarrow$ transformer.} The sources are block writes with the final
        normalization \emph{folded in}: the per-position norm scale (and LayerNorm centring) is computed
        from the full residual, frozen, and applied to each write. Three claims must be kept apart.
        \begin{enumerate}[nosep]
          ...
          \item \emph{Decision agreement} \empirical{}. The reconstructed argmax over the emitted candidate
                set (target plus top-$K$ competitors) equals the model's at every measured position,
...
$w_j=|c_j(t)|$ (the effective number of sources with sizeable target incidence);\quad
...
Welch floor $W=(n_p-d)/\bigl(d(n_p-1)\bigr)$ for $n_p>d$, else $0$. Always $\mathrm{FP}\ge W$, with
equality exactly at a unit-norm tight frame (Benedetto--Fickus). Equiangularity is an additional
condition: it is the equality case of the \emph{max}-coherence bound. These quantities depend on the
...
\begin{itemize}[nosep]
  \item \emph{The decode stratum is non-recursive and uses aggregates.} Its atoms are source facts
        ...
        $\mathrm{dec}(v)$ holds iff $A\vdash v$, which derives at most one decision atom. Sub-coalition
