...
\author{}
\date{Draft --- companion to \texttt{spec/PIC\_SPEC.md} v0.3.4}
...
matrix and taking a softmax~\cite{elhage2021mathematical}. Read literally this is a single linear map followed by a normalization; but
...
structures, machine-checked in a kernel~\cite{nipkow2002isabelle}. \emph{Learning} is the recognition that the frame is a free
...
tokens and whose variables are the sources---the tropical-polynomial picture of a ReLU network~\cite{zhang2018tropical,maclagan2015tropical}. The
...
In the transformer instance, the sources are the \emph{direct-logit-attribution (DLA) blocks}~\cite{elhage2021mathematical}: an $n_\ell$-layer model has $n_b=2n_\ell+1$
...
\paragraph{Native notation.} PIC borrows its carriers (semiring~\cite{green2007provenance}, Gram kernel,
Welch bound~\cite{welch1974lower}, power diagrams~\cite{aurenhammer1987power}) so as to inherit their theorems, but three things are native and get their own notation. The
...
The two ends are joined by \emph{Maslov dequantization}~\cite{litvinov2007maslov}: the probabilistic calculus (the model in
...
\paragraph{Semantics.} Semiring annotation in the style of Green--Karvounarakis--Tannen~\cite{green2007provenance}: the annotation of
...
\begin{itemize}[nosep]
  \item \emph{Log-semiring $\Rightarrow$ transformer.} The sources are block writes with the final
        ...
        pre-norm residual and frozen, and $\theta$ the learned gain. For RMSNorm~\cite{zhang2019root}, $\tilde d_j=s\,\theta\odot d_j$.
        For LayerNorm~\cite{ba2016layer}, $\tilde d_j=s\,\theta\odot(d_j-\operatorname{mean}(d_j))+\mathbf 1_{j=\mathrm{embed}}\,\beta$:
  ...
  \item \emph{Tropical $\Rightarrow$ power diagram.} As $T\to0$, $\arg\max_v \inner{r}{U_v}+b_v$ is
        Laguerre (power-diagram) cell membership~\cite{aurenhammer1987power} over sites $\{U_v\}$ with weights
...
ambiguity $\eta^2_k$ of a candidate rule $k$: take the fraction (default $30\%$) of training contexts with
the smallest current margin $m(t)$, and on them compute the correlation ratio of $k$'s activation with
the target label, $\eta^2_k=\mathrm{SS}_{\mathrm{between}}/\mathrm{SS}_{\mathrm{total}}$. This is the share
of the activation's variance explained by which token is the target. It is high when the rule's firing
tracks exactly the decisions that are still near a tie.
...
On real positions the \emph{centred} PR $r_{\mathrm{eff}}$ does carry signal \empirical{}. On Qwen2.5-0.5B it tracks
the greedy sufficient-coalition size beyond what the margin explains (partial Spearman $+0.35$ to $+0.47$ on prose
and code, at block and layer granularity), while raw PR tracks nothing. It is ordinal, not a count: a median
$r_{\mathrm{eff}}\approx13$ against a median sufficient coalition of 3 blocks.
\end{remark}

\paragraph{Frame-side.}
Gram coherence $\max_{v\neq w}|\rho(v,w)|$;\quad
frame potential $\mathrm{FP}=\operatorname{mean}_{v\neq w}\rho(v,w)^2$;\quad
Welch floor $W=(n_p-d)/\bigl(d(n_p-1)\bigr)$ for $n_p>d$, else $0$~\cite{welch1974lower}. Always $\mathrm{FP}\ge W$. Because FP
...
(Benedetto--Fickus~\cite{benedetto2003finite}). Equiangularity is an additional condition: it is the
equality case of the \emph{max}-coherence bound~\cite{strohmer2003grassmannian}. For $n_p\le d$, $W=0$ and $\mathrm{FP}=0$ iff the directions are pairwise
...
superposition~\cite{elhage2022superposition}. On the generator side, overpacking provably forces routing cross-talk
...
\textup{(ii)} Hence, by the classical volume-packing argument~\cite[\S4.2]{vershynin2018high}, $|V'|\le(1+2\umax/\gamma)^d$ with
...
The per-token condition, by contrast, is exact.

\begin{theorem}[Optimal margin is hull distance \proved{}]\label{thm:hull}
Let $b\equiv0$ and let $C$ be a finite, nonempty set of rivals of $t$. Then
\[
  \sup_{\lVert r\rVert\le1}\ \min_{v\in C}\ \inner{r}{U_t-U_v}
  \;=\; \operatorname{dist}\bigl(U_t,\ \operatorname{conv}\{U_v: v\in C\}\bigr),
\]
and the supremum is attained. Taking $C=V\setminus\{t\}$, $t$ is $\gamma$-decodable iff this hull distance is at
least $\gamma$. For a
unit-norm frame, with $\mu_t=\max_{v\neq t}\inner{U_t}{U_v}$,
$1-\mu_t\ \le\ \text{optimal margin}\ \le\ \sqrt{2-2\mu_t}$.
\end{theorem}
\begin{proof}[Proof sketch]
The closest hull point $p$ is a convex combination of rivals, so any unit $r$ scores some rival at most
$\inner{r}{U_t-p}\le\lVert U_t-p\rVert$. The direction $(U_t-p)/\lVert U_t-p\rVert$ attains it, by the
obtuse-angle property of closest points. The sandwich follows from $\lVert U_t-p\rVert\ge\inner{U_t-p}{U_t}$ and
from $p$ being no farther than the nearest rival. (\texttt{PIC\_Margin\_Hull.thy}:
\texttt{optimal\_margin\_hull}, \texttt{gdecodable\_iff\_hull\_dist},
\texttt{optimal\_margin\_coherence\_sandwich}.)
\end{proof}
...
\begin{remark}[The remaining link \open{}]
Theorem~\ref{thm:hull} settles the optimal margin in terms of the \emph{signed} coherence $\mu_t$. Welch
(Theorem~\ref{thm:welch}) bounds only $\lvert$coherence$\rvert$, and a large negative inner product costs
nothing, so overpacking alone does not force a \emph{named} token's optimal margin down. Closing this needs a
lower bound on the largest signed inner product among $n$ unit vectors, a spherical-code question. Trained
models pack features at $\approx$ the Welch floor with healthy margins, and the measured coherence-to-margin
link is mild.
\end{remark}
...
The one source split whose skipped part is real skipped compute is a layer \emph{prefix}, i.e.\ early exit.
Certifying it needs a bound on what the remaining layers will write. On Qwen2.5-0.5B this fails \empirical{}:
no position is certified at any exit layer, whether the suffix is bounded by its norm (even the exact norm) or by
the leftover after a J-lens prediction. The last layer's write is 50--84$\times$ the certified radius, mostly
orthogonal to the decision directions. Even an uncertified early exit could skip only about one layer.
...
certified margin mass $\#\{t:m(t)>2\delta\}$ does not decrease. Here $\delta$ is the per-step logit
perturbation bound: if a step moves each frame vector by at most $\varepsilon_U$ and each bias by at most
$\varepsilon_b$, and every tracked residual has $\lVert r\rVert\le r_{\max}$, then
$\delta=r_{\max}\varepsilon_U+\varepsilon_b$, and Theorem~\ref{thm:margin} applies with it. That a step
preserves every decision with $m(t)>2\delta$ is proved \proved{} (\texttt{pic\_learn/PIC\_Learn.thy},
\texttt{step\_decode\_preserved}, with the trajectory form \texttt{traj\_decode\_preserved}). The
monotonicity of the certified mass in (S2) is not; (S3)
...
\begin{itemize}[nosep]
  \item \emph{The decode stratum is non-recursive and uses aggregates.} Its atoms are source facts
        ...
        decision atom, and none on a tie. The emitted Souffl\'e~\cite{jordan2016souffle} program's \texttt{max}-aggregate \texttt{decide} is the
        ...
        after the layer stratum (stratified aggregation~\cite{apt1988towards}), and is not a monotone Horn
  ...
  \item \emph{The layer stratum is where recursion enters.} Its atoms $(\ell,x)$ say ``the residual at
        ...
        $(\ell{+}1,\mathrm{step}_\ell\,x)\Leftarrow(\ell,x)$. Its least model (the least fixpoint of a monotone
        operator~\cite{tarski1955lattice,vanemden1976semantics}) is exactly the forward
...
semantics~\cite{deraedt2007problog}: overlapping explanations share facts, and summing them double-counts.
Algebraic ProbLog~\cite{kimmig2011algebraic} needs extra machinery for this, which PIC does not supply. PIC's one native move is that the fact weights are
...
magic-sets/demand optimization~\cite{bancilhon1986magic} for any stratum whose operator is a monotone set operator on atoms. That
...
next-token entropy, is \emph{refuted} on a fourteen-model sweep to 72B (Pythia~\cite{biderman2023pythia}
14m--12b, Qwen2.5~\cite{qwen2024qwen25} 0.5B--72B; decision agreement $1.00$ throughout). The architecture-fair count $r_{\mathrm{eff}}$ \emph{anti}-correlates
...
\proved{} statements are machine-checked in the i-orca Isabelle/HOL~\cite{nipkow2002isabelle} corpus
...
\texttt{pic\_core/\{PIC\_Core,PIC\_Logic,PIC\_Forward,PIC\_Margin\_Hull\}.thy}, \texttt{pic\_learn/PIC\_Learn.thy},
