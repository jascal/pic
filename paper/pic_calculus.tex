...
append a coordinate carrying $b_v$ to each $U_v$ and a source $d_b$ equal to the matching unit vector.
...
atom}, the case the provenance framework is built for, arises only in the recursive fragment of
\S\ref{sec:learn} (the layer program), not in the one-step decode. The decision predicates are
...
superposition. On the frame side, however, that superposition costs no decodability, because the capacity
bound is astronomically slack for realistic $d$. On the generator side, overpacking forces routing
cross-talk (Theorem~\ref{thm:welch}). Superposition is thus present on both sides, but it \emph{binds} only on
the writers.
...
\textup{(i)} \proved{} Any set $S$ of $\gamma$-decodable tokens is a $\gamma$-code:
$\lVert U_v-U_w\rVert\ge\gamma$ for distinct $v,w\in S$ (\texttt{head\_capacity}).
\textup{(ii)} Hence, by the classical volume-packing argument, $|S|\le(1+2\rho/\gamma)^d$ with
$\rho=\max_v\lVert U_v\rVert$. The argument: disjoint radius-$\gamma/2$ balls about the points of $S$ fit
inside the ball of radius $\rho+\gamma/2$.
\end{corollary}
...
\proved{}. This is the cell-capacity half. The bound is astronomically slack for realistic $d$, so this
side rarely binds.
...
\noindent Note what Theorem~\ref{thm:headtail} partitions: \emph{tokens} ($H,T\subseteq V$), i.e.\ the
...
and needs Theorem~\ref{thm:margin} instead. Split $S=H_s\sqcup T_s$, and let
$L_{H}(v)=\llbracket H_s\rrbracket(v)+b_v$ be the head-only logit. Suppose the tail's contribution is
within $\delta$ of a common offset $c$ on every candidate, $|\llbracket T_s\rrbracket(v)-c|\le\delta$.
Then $L=L_H+\llbracket T_s\rrbracket$ is a $\delta$-perturbation of $L_H+c$, which has the same argmax
and the same margins as $L_H$. So if the head's margin exceeds $2\delta$, its decision is the model's.
...
two strata.
\begin{itemize}[nosep]
  \item \emph{The decode stratum is non-recursive.} Its atoms are source facts $\mathrm{src}(j)$ weighted by
        $j\triangleright v$ and decision atoms $\mathrm{dec}(v)$. Sources and propositions are different
        sorts, so the turnstile is the rule scheme ``$\mathrm{dec}(v)\Leftarrow\{\mathrm{src}(j):j\in P\}$
        whenever $P\vdash v$''. Its immediate-consequence operator is
        $T_P(I)=I\cup\{\mathrm{dec}(v):\exists P\subseteq\{j:\mathrm{src}(j)\in I\}.\ P\vdash v\}$. No
        $\mathrm{dec}$ atom occurs in a body, so the least fixpoint is reached in one step.
...
\end{itemize}
The semiring chooses the logic: $T=0$ is Viterbi/min-cost logic, $T=1$ is probabilistic (ProbLog-style)
logic, and Boolean is classical Datalog. PIC's one native move is that the fact weights are
\emph{derived from geometry}, $j\triangleright v=\inner{d_j}{U_v}$, rather than given.

Demand closure, $\operatorname{lfp}(\operatorname{restrict}\,T\,D)=\operatorname{lfp}T\cap D$ for a
monotone, demand-closed consequence operator, is proved \proved{} (\texttt{PIC\_Logic.thy},
...
evaluation itself is \open{}. The margin certificate (Theorem~\ref{thm:margin}) bounds how far a clause
